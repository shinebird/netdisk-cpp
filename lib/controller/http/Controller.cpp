...
namespace netdisk::controller::http
{
    void registerAll(::netdisk::core::http::Server& server)
    {
        ...
        server.addRequestHandler(post, "/login",
                                 security::request::login);
        server.addResponseHandler(post, "/login",
                                  security::response::login);
        ...
        server.addResponseHandler(get, "/user/index",
                                  response::indexPage);
        ...
        server.addResponseHandler(get, "/user/files",
                                  response::filePanel);
        server.addRequestHandler(get, "/user/downloadAndExtract",
                                 request::downloadAndExtract);
        server.addResponseHandler(get, "/user/downloadAndExtract",
                                  response::downloadAndExtract);
        server.addRequestHandler(get, "/user/batchUploadFiles",
                                 request::batchUploadFiles);
        server.addResponseHandler(get, "/user/batchUploadFiles",
                                  response::batchUploadFiles);
