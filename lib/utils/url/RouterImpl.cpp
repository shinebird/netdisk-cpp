...
namespace boost::urls::detail
{
    ...
    class segment_template
    {
        ...
        public:
            ...
            bool has_modifier() const { return !is_literal() && modifier_ != modifier::none; }
    ...
    constexpr auto path_template_rule = grammar::tuple_rule(
        ...
        grammar::range_rule(segment_template_rule,
    ...
    auto segment_template_rule_t::parse(char const*& it, char const* end) const noexcept
        ...
        if (it != end && *it == '{')
        {
            ...
            if (send != end)
            {
                ...
                static constexpr auto id_rule =
                    grammar::tuple_rule(grammar::optional_rule(arg_id_rule),
    ...
    class child_idx_vector
    {
        ...
        public:
            ...
            child_idx_vector(child_idx_vector const& other) : size_{other.size_}, cap_{other.cap_}
            {
                if (other.child_idx)
                {
    ...
    class impl
    {
        ...
        private:
            ...
            node const* try_match(segments_encoded_view::const_iterator it,
                                  segments_encoded_view::const_iterator end, node const* root,
                                  int level, core::string_view*& matches,
                                  core::string_view*& ids) const;

            // check if a node has a resource when we
            // also consider optional paths through
            // the child nodes.
            static node const* find_optional_resource(const node* root, std::vector<node> const& ns,
                                                      core::string_view*& matches,
                                                      core::string_view*& ids);
    };

    node const* impl::find_optional_resource(const node* root, std::vector<node> const& ns,
    ...
    void impl::insert_impl(core::string_view path, router_base::any_resource const* v)
    {
        ...
        while (it != end)
        {
            ...
            // look for child
            auto cit =
                std::find_if(cur->child_idx.begin(), cur->child_idx.end(),
    ...
    node const* impl::try_match(segments_encoded_view::const_iterator it,
                                segments_encoded_view::const_iterator end, node const* cur,
                                int level, core::string_view*& matches,
                                core::string_view*& ids) const
    {
        while (it != end)
    ...
    router_base::any_resource const* impl::find_impl(segments_encoded_view path,
                                                     core::string_view*& matches,
                                                     core::string_view*& ids) const
    {
        ...
        node const* p = try_match(path.begin(), path.end(), &nodes_.front(), 0, matches, ids);
        if (p)
            return p->resource;
        return nullptr;
    }
    ...
    auto router_base::find_impl(segments_encoded_view path, core::string_view*& matches,
                                core::string_view*& ids) const noexcept -> any_resource const*
    {
